...
(* Negative test: notice that the failwith message is ignored *)
...
let%test "all type preservations" =
  let open AllocatorImpl in
  let hp = make_heap 5 in
  let ptr = alloc hp 3 in
  assign_string hp ptr 0 "h";
  assign_int hp ptr 1 55;
  assign_ptr hp ptr 2 ptr;
  let res1 = deref_as_string hp ptr 0 in
  let res2 = deref_as_int hp ptr 1 in
  let res3 = deref_as_ptr hp ptr 2 in
  res1 = "h" && res2 = 55 && res3 == ptr

let%test "deref ptr of ptr" =
  let open AllocatorImpl in
  let hp = make_heap 5 in
  let ptr = alloc hp 3 in
  let ptr2 = alloc hp 2 in
  assign_ptr hp ptr 0 ptr2;
  assign_int hp ptr2 0 55;
  let res1 = deref_as_ptr hp ptr 0 in
  let res2 = deref_as_int hp res1 0 in
  res1 == ptr2 && res2 = 55

let%test "free memory" =
  let open AllocatorImpl in
  let hp = make_heap 5 in
  let ptr = alloc hp 2 in
  assign_int hp ptr 0 42;
...
let%test "test space management" =
  ...
  int_value heap (next heap n1) = 4 && int_value heap (next heap (next heap n1)) = 2
...
let%test "remove n1" =
  ...
  insert_after heap n1 n2;
  remove heap n1;
  prev heap n2 = null heap && next heap n2 = null heap
...
let%test "remove n2" =
  ...
  remove heap n2;
  x && next heap n1 = n3 && prev heap n1 = next heap n3
...
let%test "heap reclamation from SLL" =
  ...
  e1 = Some (42, "a") && e2 = Some (15, "b") && e3 = Some (33, "c")
...
let%test "can't deref a string as int" =
  ...
  assign_int hp ptr 1 35;
  try
    let _ = deref_as_int hp ptr 0 in
    false
  with
  | _ -> true
...
(* let%test "sequence of operations" =
  ...
  free hp p2 3;
  free hp p1 1;
  let i = deref_as_int hp p1 1 in
  let s = deref_as_string hp p2 3 in
  i = 35 && s = "c" *)
...
let%test "Enqueing and dequeuing before queue to list" =
  ...
  enqueue q (42, "a");
  enqueue q (462, "ab");
  dequeue q = Some (42, "a") && queue_to_list q = [(462, "ab")]
...
let%test "deref unalloc ptr" =
  ...
  let ptr = alloc hp 5 in
  try
    let _ = deref_as_ptr hp ptr 8 in
    false
  with
  | _ -> true
...
let%test "deref unalloc int" =
  ...
  let ptr = alloc hp 5 in
  try
    let _ = deref_as_int hp ptr 8 in
    false
  with
  | _ -> true
...
let%test "deref unalloc string" =
  ...
  let ptr = alloc hp 5 in
  try
    let _ = deref_as_string hp ptr 8 in
    false
  with
  | _ -> true
...
let%test "assign unalloc ptr" =
  ...
  let ptr = alloc hp 5 in
  try
    let _ = assign_ptr hp ptr 8 (null hp) in
    false
  with
  | _ -> true
...
let%test "assign unalloc int" =
  ...
  let ptr = alloc hp 5 in
  try
    let _ = assign_int hp ptr 8 5 in
    false
  with
  | _ -> true
...
let%test "assign unalloc string" =
  ...
  let ptr = alloc hp 5 in
  try
    let _ = assign_string hp ptr 8 "abc" in
    false
  with
  | _ -> true
...
let%test "pointer out of bounds deref ptr" =
  ...
  let ptr = alloc hp 10 in
  try
    let _ = deref_as_ptr hp ptr 11 in
    false
  with
  | _ -> true
...
let%test "pointer out of bounds deref int" =
  ...
  let ptr = alloc hp 10 in
  try
    let _ = deref_as_int hp ptr 11 in
    false
  with
  | _ -> true
...
let%test "pointer out of bounds deref str" =
  ...
  let ptr = alloc hp 10 in
  try
    let _ = deref_as_string hp ptr 11 in
    false
  with
  | _ -> true
...
let%test "pointer out of bounds assign ptr" =
  ...
  try
    let _ = assign_ptr hp ptr_1 11 ptr_2 in
    false
  with
  | _ -> true
...
let%test "pointer out of bounds assign int" =
  ...
  let ptr = alloc hp 10 in
  try
    let _ = assign_int hp ptr 11 5 in
    false
  with
  | _ -> true
...
let%test "pointer out of bounds assign string" =
  ...
  let ptr = alloc hp 10 in
  try
    let _ = assign_string hp ptr 11 "abc" in
    false
  with
  | _ -> true
...
let%test "null pointer deref ptr" =
  ...
  let hp = make_heap 10 in
  try
    let _ = deref_as_ptr hp (null hp) 5 in
    false
  with
  | _ -> true
...
let%test "null pointer deref int" =
  ...
  let hp = make_heap 10 in
  try
    let _ = deref_as_int hp (null hp) 5 in
    false
  with
  | _ -> true
...
let%test "null pointer deref str" =
  ...
  let hp = make_heap 10 in
  try
    let _ = deref_as_string hp (null hp) 5 in
    false
  with
  | _ -> true
...
let%test "null pointer assign ptr" =
  ...
  let ptr_2 = alloc hp 5 in
  try
    let _ = assign_ptr hp (null hp) 5 ptr_2 in
    false
  with
  | _ -> true
...
let%test "null pointer assign int" =
  ...
  let hp = make_heap 10 in
  try
    let _ = assign_int hp (null hp) 5 0 in
    false
  with
  | _ -> true
...
let%test "null pointer assign string" =
  ...
  let hp = make_heap 10 in
  try
    let _ = assign_string hp (null hp) 5 "abc" in
    false
  with
  | _ -> true
...
let%test "deref wrong type exception 1" =
  ...
  try
    let _ = deref_as_string hp ptr 0 in
    false
  with
  | _ -> true
...
let%test "FIFO characteristic of queues" =
  ...
  let rec dequeue_all n acc =
    ...
    then acc
      ...
      let res = Option.get (dequeue q) in
...
let%test "dereference wrong type0" =
  ...
  try
    let _ = deref_as_int hp x0 200 in
    false
  with
  | _ -> true
...
let%test "assigning beyond allocated memory" =
  ...
  try
    let _ = assign_int hp x0 3 52 in
    false
  with
  | _ -> true
...
let%test "cannot dereference freed memory" =
  ...
  free hp x1 2;
  (* shouldn't be derefable once freed *)
  try
    let _ = deref_as_ptr hp x1 0 in
    false
  with
  | _ -> true
...
let%test "remove" =
  ...
  and n1_next = next heap n1 in
  n1_next = n3 && n3_prev = n1
...
let%test "basic node manipulation: prev and next when there are more nodes" =
  ...
  and s_last = string_value heap last in
  i_first = 1 && s_first = "a" && i_last = 3 && s_last = "c"
